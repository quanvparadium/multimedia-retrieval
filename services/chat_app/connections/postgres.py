...
class PostgresManager:
    ...
    def create_kw_index_by_user(self, table_name, user_id: str, type_index: str = "GIN"):
        is_user_index_exists = self.index_exists(table_name=table_name, index_name=f'gin_idx_doc_by_user_{user_id}', user_id= user_id)
        ...
        create_index_query = text(f""" CREATE INDEX gin_idx_doc_by_user_{user_id} ON {table_name} USING GIN (to_tsvector('english', page_content)) WHERE (\"user_id\" = :user_id);""")         
        try:
    ...
    def create_index_by_user(self, table_name, user_id: str, type_index: str = "IVFFlat"):
        is_user_index_exists = self.index_exists(table_name=table_name, index_name=f'ivflat_idx_doc_by_user_{user_id}', user_id= user_id)
        if is_user_index_exists:
            print(f"\033[32m>>> {type_index} Index embedding by user_id({user_id}) is EXISTED.\033[0m")
            return None
        hashed_session = self.hash_session(user_id)
        create_index_query = text(f""" CREATE INDEX ivflat_idx_doc_by_user_{user_id} ON {table_name} USING ivfflat (embedding vector_cosine_ops) WITH (lists = 10) WHERE (\"user_id\" = :user_id);""") 
        try:
            for idx, engine in enumerate(self.engines):
                if idx == hashed_session:
                    with engine.connect() as connection:
